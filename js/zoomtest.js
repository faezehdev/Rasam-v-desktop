...
function imageZoom12(imgClass, resultClass) {
  ...
  img = imgClass;
  result = resultClass;
    document.querySelector('.Section-0 .img-zoom-container span').addEventListener('mouseenter',(e)=>{
      console.log(e);
      
    result.style.opacity =1;
    document.querySelector('.Section-0 .img-zoom-lens').style.opacity =1;
    document.querySelector('.Section-0 .img-zoom-result ').style.opacity =1;
})
document.querySelector('.Section-0 .img-zoom-container span').addEventListener('mouseleave',(e)=>{
  console.log(e,'ohhh');
    result.style.opacity =0;
    document.querySelector('.Section-0 .img-zoom-lens').style.opacity =0;
    document.querySelector('.Section-0 .img-zoom-result ').style.opacity =0;
})
  /*create lens:*/
...
// result.style.display ="none";
  ...
  // console.log("x" , x , "and Y " , y); 
    ...
    if (x < 0) {x = 0;}
    if (y > img.height - lens.offsetHeight) {y = img.height - lens.offsetHeight; img.addEventListener("mouseleave",  normalImg12);}//else{img.addEventListener("mouseenter", bigImg);  }
    if (y < 0) {y = 0;}
